...
function tambahData($tabel, $nama)
{
	...
	$nasabah = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT * FROM calon_penerima WHERE id='$nama'"));

	$query 	= "INSERT INTO $tabel (nama, nik, kecamatan, nagari, Usia , Pendidikan , Pendapatan , Tanggungan , Pekerjaan) VALUES ('$nasabah[nama]', '$nasabah[nik]', '$nasabah[kecamatan]', '$nasabah[nagari]' , '$nasabah[Usia]' , '$nasabah[Pendidikan]' , '$nasabah[Pendapatan]' , '$nasabah[Tanggungan]' , '$nasabah[Pekerjaan]')";
...
?>
	...
	<form class="ui form" action="proses.php" method="post">
						...
						<tr>
							...
							<td>
								<div class="field">
									...
									?>
									<input type="text" name="bobot<?php echo $urut ?>" value="<?php echo $nilai ?>" required class="form-control" style="width: 50px;">
								</div>
							</td>
						</tr>
				<?php
		...
		<br><br><input class="btn btn-success" type="submit" name="submit" value="SUBMIT" style="background-color: green; color: #fff;">
	</form>
